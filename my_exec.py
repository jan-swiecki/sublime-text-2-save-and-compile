...
class AsyncProcess(object):
    ...
    def show_panel(self):
        show_panel_on_build_stderr = sublime.load_settings("Preferences.sublime-settings").get("show_panel_on_build_stderr", True)

        if show_panel_on_build_stderr:
            self.listener.run_command("show_panel", {"panel": "output.exec"})
            # sublime.set_timeout(functools.partial(self.listener.run_command, "show_panel", {"panel": "output.exec"}), 0)

    def read_stderr(self):
        while True:
            data = os.read(self.proc.stderr.fileno(), 2**15)
            # data = os.read(self.proc.stderr.fileno(), 2)

            if data != "":
                # print "ERROR"
                # show_panel_on_build_stderr = sublime.set_timeout(functools.partial(sublime.load_settings,"Preferences.sublime-settings").get("show_panel_on_build_stderr", True))
                # print show_panel_on_build_stderr
                # if show_panel_on_build_error:
                #     # sublime.set_timeout(functools.partial(self.listener.run_command, "show_panel", {"panel": "output.exec"}), 0)
...
# Added:
#  if sublime preferences.show_panel_on_build_stderr == true:
#    on stderr show build panel
class MyExecCommand(sublime_plugin.WindowCommand, ProcessListener):
    def run(self, cmd = [], file_regex = "", line_regex = "", working_dir = "",
        ...
        self.run_command = self.window.run_command
